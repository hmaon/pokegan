...
class InstanceNormalization(Layer):
    ...
    def build(self, input_shape):
        ...
        if not self.built: # in eager mode, base_layer.py calls .build from compute_output_shape(); that's valid but we don't want to call add_weight() twice as there's no check for duplicate names?
            if self.scale:
